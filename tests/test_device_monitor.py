...
def make_event(action, subsystem, syspath, initialized=True):
	return Monitor.DeviceEvent(action, None, initialized, subsystem, None, syspath, None)
...
def test_drains_all_events():
	...
	assert dm._added == [("input", "/sys/a/event3"), ("input", "/sys/a/event4"), ("input", "/sys/a/event5")]
...
def test_rescan_adds_new_device():
	...
	class FakeEnumerator:
		def __init__(self, paths):
			self.paths, self._started = paths, False
...
def test_get_hci_addresses_closes_fd():
	...
	try:
		...
		try:
			...
			raise AssertionError("hci fd was not closed")
		except OSError:
			pass
	finally:
