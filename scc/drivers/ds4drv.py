...
from scc.drivers.evdevdrv import HAVE_EVDEV, EvdevController, get_axes, get_evdev_devices_from_syspath, make_new_device
...
class DS4Controller(HIDController):
	...
	def _load_hid_descriptor(self, config, max_size, vid, pid, test_mode):
		...
		self._decoder.axes[AxisType.AXIS_RPAD_X] = AxisData(
			...
			size=8,
			data=AxisDataUnion(
				axis=AxisModeData(button=SCButtons.RPADTOUCH, scale=1.0, offset=-127.5, clamp_max=257, deadzone=10)
			),
		)
		...
		self._decoder.buttons = ButtonData(enabled=True, byte_offset=5, bit_offset=4, size=14, button_count=14)
...
class DS4EvdevController(EvdevController):
	...
	def __init__(self, daemon, controllerdevice, gyro, touchpad):
		config = {"axes": DS4EvdevController.AXIS_MAP, "buttons": DS4EvdevController.BUTTON_MAP, "dpads": {}}
