...
from scc.constants import DAEMON_VERSION, LEFT, RIGHT, RSTICK, STICK, STICK_PAD_MAX, TRIGGER_MAX, SCButtons
...
from scc.paths import get_cache_path, get_config_path, get_profiles_path
...
class App(Gtk.Application, UserDataManager, BindingEditor):
	...
	def __init__(self, gladepath="/usr/share/scc", imagepath="/usr/share/scc/images"):
		...
		if self._dark_mode_settings is not None:
			self._dark_mode_settings.connect("notify::gtk-application-prefer-dark-theme", self.on_dark_mode_changed)
			self._dark_mode_settings.connect("notify::gtk-theme-name", self.on_dark_mode_changed)
	...
	def get_test_cursor_pixbuf(self):
		...
		return render_test_cursor(os.path.join(self.imagepath, "test-cursor.svg"), *self.get_svg_invert())
	...
	def refresh_side_icons(self):
		...
		for w in getattr(self, "button_widgets", {}).values():
			if getattr(w, "icon", None) is not None and getattr(w, "_icon_path", None):
				w.set_icon_file(w._icon_path)

	def on_dark_mode_changed(self, settings, pspec):
		"""
		System dark mode changed or the artwork needs re-evaluating.
		"""
		dark, brightness = self.get_svg_invert()
	...
	def apply_gui_config_buttons(self, config):
		...
		btDOTS = self.builder.get_object("btDOTS")
		...
		for w in (btC, btCPAD, btDPAD, btGYRO, btDOTS):
			w.set_visible(w.get_sensitive())

		# Re-layout if needed
		expected_layout = "default"
		if len(axes) >= 8 and (btC.get_sensitive() or "DOTS" in axes + buttons):
...
%s""")
	...
	def on_daemon_ccunt_changed(self, daemon, count):
		...
		switchers_by_id = {ps.get_controller().get_id(): ps for ps in old_switchers if ps.get_controller() is not None}
		old_primary = old_switchers[0] if old_switchers else None
		old_primary_controller = old_primary.get_controller() if old_primary else None
		...
		if old_primary_controller is not None and old_primary_controller.get_id() in connected_ids:
		...
		if primary_controller is not None:
			ordered_controllers = [primary_controller] + [c for c in controllers if c is not primary_controller]
			for index, (ps, c) in enumerate(zip(self.profile_switchers, ordered_controllers)):
				ps.set_controller(c)
		...
		if primary_controller is None:
			...
			self.load_gui_config_for_controller(primary_controller, first=self.controller_count == 0)
	...
	def on_daemon_error(self, daemon, error):
		...
		if "Device not found" in error:
			...
			msg += "\n" + _(
				...
				"\nor <a href='https://github.com/andersmmg/sc-controller/issues'>open issue on GitHub</a> and copy output there."
			)
	...
	def on_daemon_event_observer(self, daemon, c, what, data):
		...
		if self.osd_mode_mapper:
			...
			px, py = rotate_input_vector(data[0], data[1], self.background.get_input_rotation(area))
	...
	def check_release_notes(self):
		...
		def http_ready(f, task, buffer):
			try:
				...
				if isinstance(e, GLib.GError) and e.code == 15:  # G_IO_ERROR_NOT_SUPPORTED
					log.debug("Release notes download skipped: %s", e.message)
				else:
					log.warning("Failed to read release notes")
					log.exception(e)
