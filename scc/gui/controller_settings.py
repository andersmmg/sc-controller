...
class ControllerSettings(Editor, UserDataManager, ComboSetter):
		...
		for y in range(s):
			for x in range(s):
				...
				if a == 0 and selected and ring_inner <= d <= ring_outer:
					a = round(255 * min(1.0, ring_outer - d + 0.5, d - ring_inner + 0.5))
					data[i : i + 4] = bytes((ir_, ig_, ib_, a))
					continue
				data[i : i + 4] = bytes((r_, g_, b_, a))
		return GdkPixbuf.Pixbuf.new_from_data(bytes(data), GdkPixbuf.Colorspace.RGB, True, 8, s, s, s * 4, None, None)
	...
	def _apply_icon_settings(self, shape, color):
		...
		"""
		shape_path = icon_tint.find_base_icon(shape, self.app.imagepath) if shape else None
		if shape_path is None:
			shape = None
			shape_path = icon_tint.find_base_icon(self.controller.get_type(), self.app.imagepath)
		imgPreview = self.builder.get_object("imgPreview")
		if shape_path is not None:
			pixbuf = SVGWidget.render_cropped_svg_file(shape_path, height=85, tint=color, max_width=140)
	...
	def on_shape_changed(self, cb):
		...
		self._apply_icon_settings(self._icon_shape, icon_tint.get_icon_color(self.app.config, self.controller.get_id()))
