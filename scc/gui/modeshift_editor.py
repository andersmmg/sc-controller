...
class ModeshiftEditor(Editor):
		...
		for item, text in self.BUTTONS:
			...
			if item == SCButtons.STICKPRESS:
				if self.id == nameof(SCButtons.LPAD):
					...
					continue
			model.append(( None if item is None else nameof(item),
				None if text is None else self._input_name(item, text) ))
	...
	def on_clearb_clicked(self, trash, index, button):
		...
		for button, text in self.BUTTONS:
			model.append(( None if button is None else nameof(button),
				None if text is None else self._input_name(button, text) ))
	...
	def set_input(self, id, action, mode=None):
		...
		self.set_title(_("Modeshift for %s") % (self._input_name(id),))
