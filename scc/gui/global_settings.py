...
class GlobalSettings(Editor, UserDataManager, ComboSetter):
	...
	def _get_gamepad_icon(self, drv):
		...
		try:
			p = GdkPixbuf.Pixbuf.new_from_file(os.path.join(self.app.imagepath, "controller-icons", drv + ".svg"))
		except Exception:
	...
	def load_settings(self):
		...
		if mode not in ("system", "dark", "light"):
			mode = "system"
		(self.builder.get_object("cbTrayIconMode").set_active(("system", "dark", "light").index(mode)))
		mode = self.app.config["gui"].get("svg_invert_mode", "system")
		if mode not in ("system", "inverted", "normal"):
			mode = "system"
		(self.builder.get_object("cbSvgInvertMode").set_active(("system", "inverted", "normal").index(mode)))
		try:
			brightness = float(self.app.config["gui"].get("svg_invert_brightness", 0.8))
		except (TypeError, ValueError):
			brightness = 0.8
		self.builder.get_object("sclSvgInvertBrightness").set_value(max(40, min(100, brightness * 100)))
		self._recursing = False

		try:
			import evdev  # noqa: F401  # availability probe
		except ImportError:
	...
	def save_autoswitch_config(self):
		...
		self.app.config["autoswitch"] = [
			{"condition": row[0].condition.encode(), "action": row[0].action.to_string()} for row in tvItems.get_model()
		]
	...
	def on_cbMI_toggled(self, widget):
		...
		if self._recursing:
			return
		try:
			with open(find_menu("Default.menu")) as dm_fh:
