...
class StatusIcon(GObject.GObject):
	...
	def _get_icon_path(self):
		...
		Returns the directory the bundled icon files are kept in.
		"""
...
class StatusIconGTK3(StatusIcon):
	...
	def __init__(self, *args, **kwargs):
		...
		if not self._is_forced():
			...
			if IS_GNOME:
				log.warning("StatusIconGTK3: GNOME detected, but trying Gtk.StatusIcon anyway")
	...
	def _on_embedded_change(self, *args):
		...
		log.debug("StatusIconGTK3: embedded=%s visible=%s active=%s",
			is_embedded, self._tray.get_visible(), self.get_property("active"))
...
class StatusIconAppIndicator(StatusIconDBus):
	...
	def __init__(self, *args, **kwargs):
		...
		try:
			...
			log.warning("StatusIconAppIndicator: AppIndicator3 import failed")
			raise NotImplementedError

		category = appindicator.IndicatorCategory.APPLICATION_STATUS
		icon_path = os.path.join(self._get_icon_path(), self._get_icon() + ".svg")
		self._tray = appindicator.Indicator.new("sc-controller", icon_path, category)
		self._tray.set_menu(self._get_popupmenu())
		self._tray.set_title(self.TRAY_TITLE)

		log.info("StatusIconAppIndicator: created id=%s icon_path=%s", self._tray, icon_path)
	...
	def set(self, icon=None, text=None):
		...
		if path is not None:
			...
			self._tray.set_icon_full(name, self._get_text(text))
			log.info("StatusIconAppIndicator.set: name=%r using icon_name (file missing)", name)
...
class StatusIconProxy(StatusIcon):
	...
	def __init__(self, *args, **kwargs):
		...
		try:
			...
			log.info("StatusIcon: Using backend StatusIconGTK3 (primary)")
		except NotImplementedError:
			# Directly load fallback implementation
			log.info("StatusIcon: StatusIconGTK3 raised NotImplementedError, loading fallback")
			self._load_fallback()
