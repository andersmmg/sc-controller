...
class ProfileSwitcher(Gtk.EventBox, UserDataManager):
	...
	def set_profile(self, name, create=False):
		...
		for row in self._model:
			f = self._model.get_value(row.iter, 1)
			if f is not None:
				if name == self._model.get_value(row.iter, 0) or original_name == f.get_path():
					giofile = f
					if active == row.iter:
						# Already selected
						break
	...
	def update_icon(self):
		...
		tp = icon_tint.get_icon_shape(self.config, id, self._controller.get_type())
