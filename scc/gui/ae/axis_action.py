...
class AxisActionComponent(AEComponent, TimerManager):
	...
	def load(self):
		...
		if self.editor.get_id() in STICKS:
			# Remove "Mouse Region", "Mouse (Trackpad or Trackball)" and
			# "Mouse (Emulate Stick)" options when editing stick bindings;
			# the "Mouse" option (key 'mouse_stick') is kept for sticks
			...
			if isinstance(action, MouseAction):
				...
				if self.editor.friction > 0:
					...
					if p[0] == Axes.ABS_X and p[1] == Axes.ABS_Y:
						...
						key = "wheel_stick" if self.editor.get_id() in STICKS else "wheel_pad"
						self.set_cb(cb, key, 2)
			else:
	...
	def load_mouse_action(self, action):
		...
		if isinstance(action, MouseAction):
			key = "mouse_stick" if self.editor.get_id() in STICKS else "mouse"
			self.set_cb(cbMouseOutput, "mouse", 1)
			self.set_cb(cbAxisOutput, key, 2)
		elif isinstance(action, XYAction):
	...
	def handles(self, mode, action):
		...
		if isinstance(action, XYAction):
			...
			if (p[0] == Axes.ABS_X and p[1] == Axes.ABS_Y) or (p[0] == Axes.ABS_RX and p[1] == Axes.ABS_RY):
				return True
			if (p[0] == Axes.ABS_HAT0X and p[1] == Axes.ABS_HAT0Y) or (
				p[0] == Rels.REL_HWHEEL and p[1] == Rels.REL_WHEEL
			):
				return True
		return False
	...
	def set_joystick_round(self, action):
		...
		"""
		self._recursing = True
		self.builder.get_object("cbJoystickRound").set_active(bool(getattr(action, "normalize", False)))
		self._recursing = False
...
class FakeMapper:
	...
	def get_current_window(self):
		...
		NET_WM_WINDOW_TYPE_NORMAL = X.intern_atom(self._xdisplay, "_NET_WM_WINDOW_TYPE_NORMAL", False)
		my_windows = [x.get_xid() for x in Gdk.Screen.get_default().get_toplevel_windows()]
		nitems, prop = X.get_window_prop(self._xdisplay, root, "_NET_CLIENT_LIST_STACKING", max_size=0x8000)
		...
		if nitems > 0:
			for i in reversed(range(0, nitems)):
				...
				if not X.is_window_visible(self._xdisplay, window):
					...
					continue
				tp = X.get_window_prop(self._xdisplay, window, "_NET_WM_WINDOW_TYPE")[-1]
				if tp is not None:
