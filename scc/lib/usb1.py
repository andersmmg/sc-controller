...
class USBTransfer:
	...
	def __init__(self, handle, iso_packets, before_submit, after_completion):
		...
		self.__ctypesCallbackWrapper = libusb1.libusb_transfer_cb_fn_p(self.__callbackWrapper)
	...
	def setControl(self, request_type, request, value, index, buffer_or_len, callback=None, user_data=None, timeout=0):
		...
		libusb1.libusb_fill_control_transfer(
			self.__transfer, self.__handle, string_buffer, self.__ctypesCallbackWrapper, None, timeout
		)
		self.__callback = callback
	...
	def setIsochronous(
		self, endpoint, buffer_or_len, callback=None, user_data=None, timeout=0, iso_transfer_length_list=None
	):
		"""
...
class USBDeviceHandle:
	...
	def getRawDescriptor(self, descriptor, desc_index, length):
		...
		data = (c_uint8 * length)()
		try:
			mayRaiseUSBError(libusb1.libusb_get_descriptor(self.__handle, descriptor, desc_index, data, length))
		# pylint: disable=undefined-variable
		except USBErrorNotFound:
			# pylint: enable=undefined-variable
			return None
...
class USBDevice:
	...
	def __getitem__(self, index):
		return USBConfiguration(self.__context, self.__configuration_descriptor_list[index])
...
class USBContext:
	...
	def getDeviceIterator(self, skip_on_error=False):
		...
		device_list_len = libusb1.libusb_get_device_list(self.__context_p, byref(device_p_p))
	...
	def setPollFDNotifiers(self, added_cb=None, removed_cb=None, user_data=None):
		...
		self.__libusb_set_pollfd_notifiers(self.__context_p, added_cb, removed_cb, user_data)
