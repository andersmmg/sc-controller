...
class libusb_interface(Structure):
	_fields_ = [("altsetting", libusb_interface_descriptor_p), ("num_altsetting", c_int)]
...
class libusb_iso_packet_descriptor(Structure):
	_fields_ = [("length", c_uint), ("actual_length", c_uint), ("status", c_int)]  # enum libusb_transfer_status
...
if "FreeBSD" in platform.system() and getattr(libusb, "libusb_get_string_descriptor", None) is None:
...
libusb_get_active_config_descriptor.argtypes = [libusb_device_p, libusb_config_descriptor_p_p]
# int libusb_get_config_descriptor(libusb_device *dev, uint8_t config_index,
#        struct libusb_config_descriptor **config);
libusb_get_config_descriptor = libusb.libusb_get_config_descriptor
libusb_get_config_descriptor.argtypes = [libusb_device_p, c_uint8, libusb_config_descriptor_p_p]
# int libusb_get_config_descriptor_by_value(libusb_device *dev,
#        uint8_t bConfigurationValue, struct libusb_config_descriptor **config);
libusb_get_config_descriptor_by_value = libusb.libusb_get_config_descriptor_by_value
libusb_get_config_descriptor_by_value.argtypes = [libusb_device_p, c_uint8, libusb_config_descriptor_p_p]
...
def libusb_fill_iso_transfer(
	transfer_p, dev_handle, endpoint, buffer, length, num_iso_packets, callback, user_data, timeout
):
	transfer = transfer_p.contents
	transfer.dev_handle = dev_handle
	transfer.endpoint = endpoint
...
libusb_bulk_transfer.argtypes = [libusb_device_handle_p, c_uchar, c_void_p, c_int, c_int_p, c_uint]

# int libusb_interrupt_transfer(libusb_device_handle *dev_handle,
#        unsigned char endpoint, unsigned char *data, int length,
#        int *actual_length, unsigned int timeout);
libusb_interrupt_transfer = libusb.libusb_interrupt_transfer
libusb_interrupt_transfer.argtypes = [libusb_device_handle_p, c_uchar, c_void_p, c_int, c_int_p, c_uint]


# pylint: disable=undefined-variable
def libusb_get_descriptor(dev, desc_type, desc_index, data, length):
	return libusb_control_transfer(
		dev, LIBUSB_ENDPOINT_IN, LIBUSB_REQUEST_GET_DESCRIPTOR, (desc_type << 8) | desc_index, 0, data, length, 1000
	)
...
libusb_get_string_descriptor_ascii.argtypes = [libusb_device_handle_p, c_uint8, c_void_p, c_int]
